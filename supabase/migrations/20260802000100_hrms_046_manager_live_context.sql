-- HRMS-046: expose current project/activity context in the company live board
-- to Manager, Admin and Superadmin roles while preserving Employee project scope.

BEGIN;

CREATE OR REPLACE FUNCTION public.live_work_status()
RETURNS TABLE (
  employee_id UUID,
  employee_name TEXT,
  employee_code TEXT,
...
AS $$
  SELECT
    ...
    CASE
      ...
      ELSE GREATEST(
        ...
        COALESCE(latest_closed_break.ended_at, open_entry.started_at)
      )
  ...
  LEFT JOIN LATERAL (
    SELECT break_entry.ended_at
    FROM public.break_entries break_entry
    WHERE break_entry.work_entry_id = open_entry.id
      AND break_entry.ended_at IS NOT NULL
    ORDER BY break_entry.ended_at DESC
    LIMIT 1
  ) latest_closed_break ON true
  LEFT JOIN LATERAL (
